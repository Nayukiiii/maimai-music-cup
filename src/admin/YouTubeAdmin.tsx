...
export default function AdminApp() {
  ...
  function exportJson() {
    const ordered = Object.fromEntries(
      songs.flatMap((song) => draft[song.id] ? [[song.id, draft[song.id]] as const] : []).sort(([a], [b]) => a.localeCompare(b))
    );
  ...
  async function importJson(file: File) {
    ...
    try {
      ...
      const knownEntries = Object.entries(parsed).filter(([songId]) => knownIds.has(songId));
      const ignored = Object.keys(parsed).length - knownEntries.length;
      persist(
        { ...draft, ...Object.fromEntries(knownEntries) },
        `导入成功：合并 ${knownEntries.length} 条映射${ignored ? `，忽略 ${ignored} 条未知歌曲 ID` : ""}。`
      );
    } catch (error) {
